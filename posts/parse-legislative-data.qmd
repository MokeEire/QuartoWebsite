...
Data comes in many forms. You have probably used data formats like Excel spreadsheets, text files, relational databases and if you're reading this you may have run into some (e)Xtensible Markup Language, or XML files. XML is a storage format designed for flexibly transmitting data[^1] but this flexibility often leads to a bit of a hassle when trying to parse it for use in analytics. This article takes a look at this tricky, but valuable data type, how to use it, and a set of steps for processing XML with R so it can be prepared for .
...
As always the first step of data analysis is to check if any documentation exists and thankfully [govinfo](https://www.govinfo.gov/) has provided some [guidance](https://github.com/usgpo/bill-status/blob/main/BILLSTATUS-XML_User_User-Guide.md) for how to interpret the data and where its limitations are.
...
Where this action `<item>` node is one of many actions taken on a bill (its *parent* node), and may also contain multiple committee `<item>` nodes (its *children*). To have a table at this level of detail could end up being hundreds of rows per bill, which makes this flexible structure more helpful.

# Read XML data with R

We'll load the `{XML}` and `{xml2}` packages to read in our XML file. Between these two libraries there are numerous ways to load in and process XML data. I strongly suggest looking through the packages' help files to find the right function for your task. I usually also load the `{tidyverse}` package because it is packed with useful functions.
...
Read in the file with `read_xml()` , which returns an `xml_document`[^2] object. Now check what's inside.
...
But notice that a number of fields like `actions`, `committees`, and `sponsors` have had their values concatenated. In our case we need to do a little bit more work to get the data into a tidy dataframe, but this approach can work for simpler XML structures. The importance of understanding the structure of your data brings us to one of the cardinal rules of data analysis:
...
This is where the majority of the legwork is so if you can comfortably ignore[^4] some portion of the data, now's a good time. There's no point spending an hour trying to write a function for one particularly difficult bit of information only to realize you don't need it anyway.
...
For brevity I will just go through actions and votes, but you can find the code for extracting the full XML file [here](https://github.com/MokeEire/my-reps/blob/master/R/parsing_functions.R)[^5].
